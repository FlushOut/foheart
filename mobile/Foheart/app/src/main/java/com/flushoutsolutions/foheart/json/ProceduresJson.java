 ...
 */
public class ProceduresJson {
    private JSONArray jsonArray;

    public ProceduresJson(String appCode) throws IOException, JSONException
    {
        SharedPreferences settings = FoHeart.getAppContext().getSharedPreferences("userconfigs", 0);

        File file = new File(FoHeart.getAppContext().getApplicationInfo().dataDir+"/apps/app"+appCode+"/app/procedures.json");
        InputStream is = null;
        try
        {
